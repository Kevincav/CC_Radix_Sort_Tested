...
class HistogramTest : public ::testing::Test {
 protected:
  virtual void SetUp() { hist_.reset(new Histogram); }
...
TEST_F(HistogramTest, TestFlipFlopForIntegers32Bit) {
  // Tests that negative value will be between [0.0.0.0, 127.255.255.255] when
  // flipped.
  // Tests that zero value will be 128.0.0.0 when flipped.
  // Tests that positive value will be between [128.0.0.0, 255.255.255.255] when
  // flipped.
...
TEST_F(HistogramTest, TestGetHistogramFor8BitArrayEmpty) {
  ...
  std::vector<uint64_t> output =
      hist_->GetHistogram(&input[0], input.size(), UNSIGNED);
...
TEST_F(HistogramTest, TestGetHistogramFor8BitArraySigned) {
  ...
  std::vector<char> input(
      {-127, -43, 0, 13, 27, 123, 127, -113, -18, 123, 13, -43});
  uint8_t *input_revised = reinterpret_cast<uint8_t *>(&input[0]);
  std::vector<uint64_t> expected(256, 0);
  ...
  hist_->GetPrefixSum(expected);
  std::vector<uint64_t> output =
      hist_->GetHistogram(input_revised, input.size(), SIGNED);
  EXPECT_EQ(expected, output);
}
...
TEST_F(HistogramTest, TestGetHistogramFor16BitArraySigned) {
  ...
  std::vector<int16_t> input(
      {-127, -43, 0, 13, 27, 12345, 127, -20003, -18, 123, 13, -43});
  uint16_t *input_revised = reinterpret_cast<uint16_t *>(&input[0]);
  std::vector<uint64_t> expected(65536, 0);
  for (int i = 0; i < input.size(); ++i) {
    ++expected[hist_->FlipFlopInteger(input_revised[i])];
  }
...
TEST_F(HistogramTest, TestGetHistogramFor32BitArrayUnsigned) {
  ...
  std::vector<uint32_t> input(
      {0, 13, 27, 123, 182, 2232392824, 255, 123232452, 13});
  std::vector<std::vector<uint64_t>> expected(3,
                                              std::vector<uint64_t>(2048, 0));
  for (auto &value : input) {
...
TEST_F(HistogramTest, TestGetHistogramFor32BitArraySigned) {
  ...
  std::vector<int> input({-1279248232, -43, 0, 13, 27242324, 12345, 127,
                          -20003249, -18, 123, 13, -432424234});
  uint32_t *input_revised = reinterpret_cast<uint32_t *>(&input[0]);
  std::vector<std::vector<uint64_t>> expected(3,
                                              std::vector<uint64_t>(2048, 0));
  for (int i = 0; i < input.size(); ++i) {
...
TEST_F(HistogramTest, TestGetHistogramFor32BitArrayFloat) {
  ...
  std::vector<float> input({-1279248232.2924, 0.0000001, 13.0242, 27282324.0,
                            12345.0, -20003242449.0, -18.813814, 123.0, 13.0,
                            -4324244.0});
  uint32_t *input_revised = reinterpret_cast<uint32_t *>(&input[0]);
  std::vector<std::vector<uint64_t>> expected(3,
                                              std::vector<uint64_t>(2048, 0));
  ...
  std::vector<std::vector<uint64_t>> output =
      hist_->GetHistogram(input_revised, input.size(), FLOAT);
  EXPECT_EQ(expected, output);
}
...
TEST_F(HistogramTest, TestGetHistogramFor64BitArrayEmpty) {
  ...
  std::vector<std::vector<uint64_t>> expected(6,
                                              std::vector<uint64_t>(2048, 0));
  std::vector<std::vector<uint64_t>> output =
      hist_->GetHistogram(&input[0], input.size(), UNSIGNED);
  EXPECT_EQ(expected, output);
...
TEST_F(HistogramTest, TestGetHistogramFor64BitArrayUnsigned) {
  ...
  std::vector<uint64_t> input(
      {0, 13, 272409240842082048, 123, 182, 2232392824, 255, 123232452, 13});
  std::vector<std::vector<uint64_t>> expected(6,
                                              std::vector<uint64_t>(2048, 0));
  for (auto &value : input) {
    ++expected[0][hist_->ExtractBit(value, 0)];
...
TEST_F(HistogramTest, TestGetHistogramFor64BitArraySigned) {
  ...
  std::vector<int64_t> input({-1279248232, -43, 0, 13, 27242324, 12345, 127,
                              -2000324924242424, -18, 123, 13, -432424234});
  uint64_t *input_revised = reinterpret_cast<uint64_t *>(&input[0]);
  std::vector<std::vector<uint64_t>> expected(6,
                                              std::vector<uint64_t>(2048, 0));
  for (int i = 0; i < input.size(); ++i) {
...
TEST_F(HistogramTest, TestGetHistogramFor64BitArrayFloat) {
  ...
  std::vector<double> input({-1279248224249232.29223234, 0.0000001, 13.0242,
                             27282324.0, 12345.0, 123.0, 13.0, -4324244.0});
  uint64_t *input_revised = reinterpret_cast<uint64_t *>(&input[0]);
  std::vector<std::vector<uint64_t>> expected(6,
                                              std::vector<uint64_t>(2048, 0));
  for (int i = 0; i < input.size(); ++i) {
