...
RadixSort::RadixSort() {
    histogram_.reset(new Histogram);
}
...
void RadixSort::SortType(uint32_t* array, const int size, const enum SortType type) {
    // Sort all 32-bit data types based on uint32_t bit structure.
    if (size == 0) {
        return;
    }
    std::vector<std::vector<kHistogramDataType>> int_hist = histogram_->GetHistogram(array, size, type);
    std::vector<uint32_t> placeholder_array(size);
    for (int i = size - 1; i >= 0; --i) {
        placeholder_array[--int_hist[0][histogram_->ExtractBit(array[i], 0)]] = array[i];
    }
    for (int i = size - 1; i >= 0; --i) {
        array[--int_hist[1][histogram_->ExtractBit(placeholder_array[i], 1)]] = placeholder_array[i];
    }
    for (int i = size - 1; i >= 0; --i) {
        placeholder_array[--int_hist[2][histogram_->ExtractBit(array[i], 2)]] = array[i];
    }
...
void RadixSort::SortType(uint64_t* array, const int size, const enum SortType type) {
    // Sort all 64-bit data types based on uint64_t bit structure.
    if (size == 0) {
        return;
    }
    std::vector<std::vector<kHistogramDataType>> ULL_hist = histogram_->GetHistogram(array, size, type);
    std::vector<uint64_t> placeholder_array(size);
    for (int i = size - 1; i >= 0; --i) {
        placeholder_array[--ULL_hist[0][histogram_->ExtractBit(array[i], 0)]] = array[i];
    }
    for (int i = size - 1; i >= 0; --i) {
        array[--ULL_hist[1][histogram_->ExtractBit(placeholder_array[i], 1)]] = placeholder_array[i];
    }
    for (int i = size - 1; i >= 0; --i) {
        placeholder_array[--ULL_hist[2][histogram_->ExtractBit(array[i], 2)]] = array[i];
    }
    for (int i = size - 1; i >= 0; --i) {
        array[--ULL_hist[3][histogram_->ExtractBit(placeholder_array[i], 3)]] = placeholder_array[i];
    }
    for (int i = size - 1; i >= 0; --i) {
        placeholder_array[--ULL_hist[4][histogram_->ExtractBit(array[i], 4)]] = array[i];
    }
    if (type == UNSIGNED) {  // No Flip Flop.
        for (int i = size - 1; i >= 0; --i) {
            array[--ULL_hist[5][histogram_->ExtractBit(placeholder_array[i], 5)]] = placeholder_array[i];
        }
    } else if (type == SIGNED) {  // Use FlipFlopInteger.
        for (int i = size - 1; i >= 0; --i) {
            array[--ULL_hist[5][histogram_->ExtractBit(placeholder_array[i], 5)]] = histogram_->FlipFlopInteger(placeholder_array[i]);
        }
    } else {  // Use FlopFloatingPoint.
        for (int i = size - 1; i >= 0; --i) {
            array[--ULL_hist[5][histogram_->ExtractBit(placeholder_array[i], 5)]] = histogram_->FlopFloatingPoint(placeholder_array[i]);
        }
    }
}
